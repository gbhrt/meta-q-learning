...
class CarRacingSimple1(gym.Env, EzPickle):
    ...
    def _reset(self, acc_factor = 1.0, target_distance_vec = [20.0], target_velocity_vec = [5.0]): #
        ...
        if self.render_flag:
            ...
            if len(self.acc_vec) > 1:
                with open(self.acc_filename, "a") as f:
                    writer = csv.writer(f)
                    writer.writerow([self.acc_factor] + [self.target_distance_vec[0]] + [self.target_velocity_vec[0]] + self.acc_vec)
        ...
        self.x = 0
        self.vel = 0
        # self.target = [0.0,self.target_distance_vec[0],self.target_velocity_vec[0]] #x,y,vel
    ...
    def _step(self, action):
        ...
        target_pos = np.array(self.target[1])
        target_vel = np.array(self.target[2])
        # distance_to_target = np.linalg.norm(target_pos - self.x)
        distance_to_target = target_pos - self.x 
        ...
        if action is not None:  # First step without action, called from reset()
            ...
            step_reward = self.vel/1000 - 0.1*(action[0] - self.last_action[0])**2# is working
            ...
            if distance_to_target < self.min_dis_to_target:
                ...
                step_reward = 0.0 if self.vel < 5.0 else - ((self.vel -target_vel)*0.75)**2 # 
