    ...
    def _contact(self, contact, begin):
        ...
        if begin:
            ...
            if not tile.road_visited:
                ...
                self.env.reward += 1.0 #1000.0 / len(self.env.track)
                self.env.tile_visited_count += 1
        else:
            obj.tiles.remove(tile)


class CarRacingSimple(gym.Env, EzPickle):
    metadata = {
    ...
    def _reset(self, friction_scale = 1.0): #
        self._destroy()
        self.reward = 0.0
        self.prev_reward = 0.0
        self.tile_visited_count = 0
        ...
        init_y = 0
        init_ang = 0#np.pi/2
        self.target = [0.0,20.0,0.0] #x,y,vel
        self.car = Car(self.world,init_x,init_y,init_ang,friction_scale) #
        # self.car = Car(self.world, *self.track[0][1:4])
...
if __name__ == "__main__":
    ...
    #         a[1] = +0.01#1.0
    #     if k == key.DOWN:
    #         a[2] = +0.01# 0.8  # set 1.0 for wheels to block to zero rotation
    ...
    def key_press(k, mod):
        ...
        if k == key.UP:
            a[0] = +acc_command#1.0
        if k == key.DOWN:
            a[0] = -acc_command# 0.8  # set 1.0 for wheels to block to zero rotation

    def key_release(k, mod):
        if k == key.DOWN: # and a[0] == -acc_command:
            a[0] = 0
        if k == key.UP: # and a[0] == +acc_command:
            a[0] = 0
    env = CarRacingSimple()
    env.render()
