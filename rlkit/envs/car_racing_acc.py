...
class CarRacing(CarRacingSimple):
    ...
    def sample_tasks(self, num_tasks):
        ...
        target_distance_vec_vec = np.random.uniform(10.0, 40.0, size=(num_tasks,1))
        for task,target_distance_vec in zip(tasks,target_distance_vec_vec):
            task['target_distance_vec'] = target_distance_vec

        target_velocity_vec_vec = np.random.uniform(5.0, 20, size=(num_tasks,1))
    ...
    def reset(self):
        return self._reset(self._acc_factor,self._target_distance_vec,self._target_velocity_vec)
